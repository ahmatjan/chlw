...
public class CustomMessageAction extends BaseAction {
	...
	public String Custom(){
		try {
			...
			String userNo =js.getString("value");
			String md5code = userNo + TuserinfoAction.INTERFACE_KEY;
			...
			String re = JSONReslutUtil.getResultMessage(
					ResourceBundleUtil.SENDURL + "/Interface!createCustomMessage?",
					"custom="+obj+"&userno=" + userNo + "&Mac=" + md5 + "&interfaceType=002", "POST");
		...
		try{
			...
			if(!LotErrorCode.NEW_OK.trim().equals(obj.getString("errorCode").trim())){
				...
				String count = JSONReslutUtil.getResultMessage(
						ResourceBundleUtil.LINKSMSCOUNTURL + "/phoneInfo!getContNew?",
						"emailCode=Msg" +userno+"&rules=1" , "POST");
