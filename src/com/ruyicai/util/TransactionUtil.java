...
//			e.printStackTrace();
	 ...
	 * @param accessType
	 * @param cardtypeId
	 * @return@Sql("select * from v_cardmap where accesstype=? and cardtypeid
	 *                     like ?")
	 * 
	 *                     public static String getFreerate(String accessType
	 *                     ,String cardtypeId){ //查询费率表 StringBuffer sbf = new
	 *                     StringBuffer(); StringBuffer sbf1= new
	 *                     StringBuffer(); StringBuffer sbf2= new
	 *                     StringBuffer(); List<CardMap> listCardMap =
	 *                     Global.issueDAO.getCardMapByCardId(accessType
	 *                     ,cardtypeId); if(listCardMap!=null &&
	 *                     listCardMap.size()>0){ for(int
	 *                     i=0;i<listCardMap.size();i++){
	 *                     //移动、联通、电信充值卡4%手续费，骏网一卡通手续费18%,征途和盛大游戏卡手续费20% CardMap
	 *                     cardMap = listCardMap.get(i);
	 *                     if(cardMap.getTargetcardId().equals("CMJFK"))
	 *                     sbf.append(cardMap.getCardName()+"、");
	 *                     if(cardMap.getTargetcardId().equals("LTJFK"))
	 *                     sbf.append(cardMap.getCardName()+"、");
	 *                     if(cardMap.getTargetcardId().equals("DXJFK"))
	 *                     sbf.append
	 *                     (cardMap.getCardName()+cardMap.getFeerate()/
	 *                     100+"%手续费，");
	 *                     if(cardMap.getTargetcardId().equals("JUNNET"))
	 *                     sbf2.append
	 *                     (cardMap.getCardName()+"手续费"+cardMap.getFeerate
	 *                     ()/100+"%");
	 *                     if(cardMap.getTargetcardId().equals("ZHENGTU"))
	 *                     sbf1.append
	 *                     (cardMap.getCardName()+"手续费"+cardMap.getFeerate
	 *                     ()/100+"%，");
	 *                     if(cardMap.getTargetcardId().equals("SNDACARD"))
	 *                     sbf1.append(cardMap.getCardName()+"和");
	 * 
	 *                     } System.out.println("查询数据库得到的费率为:"+sbf+sbf1+sbf2);
	 *                     return
	 *                     sbf.toString()+sbf1.toString()+sbf2.toString(); }
	 *                     return null; }
	 */
}
