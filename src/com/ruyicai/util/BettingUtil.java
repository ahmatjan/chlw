...
import com.ruyicai.web.action.cooperation.dto_Sales;
		...
		if("hemai".equals(request.getParameter("daiGou"))&&clrJson.getLong("buyAmt")+clrJson.getLong("safeAmt")<=depositMoney){
			...
			if("hemai".equals(request.getParameter("daiGou"))){
				...
				if(!"中".equals(clrJson.getString("isIe"))){
					desc = com.jrt.invokeLot.util.URLEncoder.decode(desc);
				}
				clr.setDesc(desc);
...
//		if(totalMoney <= depositMoney){
			...
			//调用订单投注的方法
			String aa= ResourceBundleUtil.LINKURL +"/present/savepresent?body="+JSONObject.fromObject(orderRequest);
...
//			    Date toDate=dateFormat.parse(qihaoStrarry[i].split(",")[3]);
	...
	public static String getMemo(String wanfa,String lotno, String betcode){
		String memo = "";
		String wanfas[];
		if(lotno.equals(Constant.JCZQ_SPF)){
			wanfas = wanfa.split("-");
		}else{
			wanfas= wanfa.split("\\+");
		}
	...
	public static JSONObject getLotnoBetType2(String wanfa,String lotno,String betcode){
		...
		}else if(Constant.JCZQ_SPF.equals(lotno)){// 竞彩胜平负
			if("500".equals(wanfa)){
				 memo = "0";
			}else{
				 memo = "1";
	...
	private static String getCodesByLotno(String str,String path,String lotno){
    	 ...
    	 }else if(lotno.equals(Constant.JCZQ_SPF)) {
			 str = addCommas(str);
			 str = str.replace(",", "|");
    	 }else{//其他彩种
			 str = str.trim().replace(",", "");
	 ...
	 * @throws IOException 
	 * 
	 * 福彩	userno  lotNo batchNum amount betCode 
	 * 
	 * 体彩
	 * batchCode//期号	lotMulti  //倍数   batchNum //追号数	oneAmount //oneMoney 追加为3元	
	 * userno //用户编号 amount	//总金额  betCode //注码
	 */
	public static JSONObject getChargesBet(String userno,String lotno,
			String betcode,String batchNum,int multiple,String batchCode,
			String oneAmount,HttpServletRequest request) throws Exception{
		boolean superaddition = false;
		if(oneAmount.trim().equals("3")){
			superaddition=true;
		}
		
		JSONObject objReturn = null;
		List<BetcodeBean> list = getBetcodeBeanList(lotno, betcode, multiple,superaddition);
		logger.info("要投注的注码一共有" + list.size() + "张彩票");
		
		JSONObject objBalance = CommonUtil.fiandBalance(request);
		double depositMoney = objBalance.getDouble("balance");//可投注金额//Integer.parseInt(comp.getDeposit_amount())/100;
		logger.info("账户可投注金额为:"+depositMoney);

		int totalMoney = 0;//用户投注的金额
		for(BetcodeBean betcodeBean : list){
			totalMoney += Integer.parseInt(betcodeBean.getTotalMoney());
		}
		logger.info("用户要投注金额:"+totalMoney);
		
		//判断投注的金额是否大于实际的投注余额
		if(totalMoney <= depositMoney){	
			String params ="userno="+userno+"&lotNo="+lotno+"&batchNum="+batchNum
			+"&batchCode="+batchCode+"&lotMulti="+multiple+"&oneAmount="+oneAmount;
			String url = ResourceBundleUtil.LINKURL+"/tlots/getTlot?";//拼接的url

			
			//调用接口执行投注
			for (BetcodeBean betcodeBean : list) {
					params+="&amount="+betcodeBean.getTotalMoneyFen()+"&betCode="+betcodeBean.getBetcode();	
			}
			
			//调用post方法调用后台接口返回交易号
			String re = JSONReslutUtil.getResultMessage(url, params, "POST");
			JSONObject jsonValue = JSONObject.fromObject(re);
			if(jsonValue!=null){
				String error_code = CommonUtil.getBackValue("errorCode", jsonValue);	
				if(error_code.trim().equals(LotErrorCode.NEW_OK.trim())){
					//将订单号码放到json中返回
					objReturn =  JSONObject.fromObject(CommonUtil.getBackValue("value", jsonValue));
					objReturn.put("transaction_money", totalMoney);
				}
			}
...
//				betcode = Constant.ZC_SFC + Constant.ZC_FS + betcode;
//			}else {
//				betcode = Constant.ZC_SFC + Constant.ZC_DS + betcode;
//			}
...
//			if(betcode.contains("$")) {
//				betcode = Constant.ZC_RJC + Constant.ZC_DT + betcode;
//			}else if(betcode.length()>27) {
//				betcode = Constant.ZC_RJC + Constant.ZC_FS + betcode;
//			}else {
//				betcode = Constant.ZC_RJC + Constant.ZC_DS + betcode;
//			}
			list = ZCResolveService.getZCBetcodeList(betcode, multiple,
					Constant.TC_TABNUMBER, Constant.SIGN, Constant.DT_TAB,
					Constant.ZC_STREAK);
...
//				betcode = Constant.ZC_JQC + Constant.ZC_FS + betcode;
//			}else {
//				betcode = Constant.ZC_JQC + Constant.ZC_DS + betcode;
//			}
			list = ZCResolveService.getZCBetcodeList(betcode, multiple,
					Constant.TC_TABNUMBER, Constant.SIGN, Constant.DT_TAB,
					Constant.ZC_STREAK);
...
//				betcode = Constant.ZC_BQC + Constant.ZC_FS + betcode;
//			}else {
//				betcode = Constant.ZC_BQC + Constant.ZC_DS + betcode;
//			}
			list = ZCResolveService.getZCBetcodeList(betcode, multiple,
					Constant.TC_TABNUMBER, Constant.SIGN, Constant.DT_TAB,
					Constant.ZC_STREAK);
			...
			list = SYYDJResolveService.getSYYDJBetcodeList(betcode, multiple, 
					Constant.TC_TABNUMBER, Constant.QH_TAB, Constant.SIGN, 
					Constant.DT_TAB);
		}else if(Constant.JCZQ_SPF.equals(lotno)) {//竞彩足球胜平负
			list = JCResolveService.getJCBetcodeList(betcode, multiple,
