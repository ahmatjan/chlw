...
public class UtilGetEmailUrl {
	...
	public static String getEmailUrl(String mailLocal){
		...
		try {
			...
			if( blaen && state<400){
				System.out.println("call http://mail."+mailLocal+" success");
...
//			e.printStackTrace();
		...
		if(blaen){
			try {
				...
				if(state<400){
					System.out.println("call http://"+mailLocal+" success");
					emailUrl = mailLocal;
					blaen = false;
				}
			} catch (MalformedURLException e) {
	//			e.printStackTrace();
		...
		if(blaen){
			try {
				...
				if(state<400){
					System.out.println("call http://www."+mailLocal+" success");
					emailUrl = "www."+mailLocal;
				}
				else{
					System.out.println("call all mail  www "+mailLocal+" error");
				}
			} catch (MalformedURLException e) {
	//			e.printStackTrace();
			} catch (IOException e) {
        ...
        System.out.println("return : "+emailUrl);
		return emailUrl;
	}
	
	public static void main(String[] args) {
		UtilGetEmailUrl.getEmailUrl("gmail.com");
	}
