...
<style>div.My_account{ width:778px; padding:0px;} div.My_account h2{ margin:0 14px;}</style>
			<h2>积分管理</h2>
				...
				<ul class="account_rechargeTab" >
					<li ControlTarget=".cash_Records" class="light selected" onclick="BaseTab($(this));">积分明细</li>
					<li ControlTarget=".user_Drawing" class="light" onclick="BaseTab($(this));">积分兑换</li>
				</ul>
				<span class=""><a  href="http://www.ruyicai.com/bangzhuzhongxin/goucaibangzhu/jifen/20120329114128_7138.html?id=7138&code1=209&code=126" target="_blank">如何获得更多积分？</a></span>
			</div>
			<div class="account_rechargeContent">
			<!--提款记录-->
				<div class="cash_Records none selected">
				<form action="/chlw/user/tuserinfoAction!accountDetail" method="get" onsubmit="return reHtmlInParameters('',this.startDate.value,this.stopDate.value,'')">
					...
					<table class="account_day"  cellspacing="0" cellpadding="0">
						...
						<tbody>
				<s:if test="#request.list.today.size()+#request.list.yestday.size()+#request.list.oldday.size()==0">
           			<tr><td colspan="9" align="center" height="28">没有查询到积分变动记录</td></tr>
                </s:if>
               <s:else>
               <%//-----------------------------今天的账户变动记录----------------------------------------------------- %>
				...
				<s:else><tr class="accountDay_greyBg"></s:else>
					...
					<%//-- 积分变动 --%>
					<td><s:property value="scoreTypeMsg"/>
					<s:if test='scoreTypeMsg=="+"'>
							<b><s:property value="score"/></b>
						</s:if>
						<s:elseif test='scoreTypeMsg=="-"'>
			...
			<!--提款记录end-->
			<!--用户提款-->
			<script>getUserScore("scores");</script>
				<div class="user_Drawing none">
				<p class="userDrawing_p">总积分：<b id="scores">0</b></p>
				<table >
					  <tr>
						<td width="90">要兑换的积分：</td>
						<td width="80"><input name="" type="text" class="myExchang" id="socoreNum"/></td>
						<td width="70" style="display:none">个积分兑换</td>
						<td width="80" style="display:none"><input name="" type="text" class="myExchang" /></td>
						<td width="55" style="display:none">元购彩金</td>
						<td><input type="button" class="ExchangeBtn"  value="兑换" onclick='makescoreToM("socoreNum");' /></td>
					  </tr>
					  <tr>
