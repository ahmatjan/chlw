	...
	<tr>
		...
		<td>
			<select name="" class="bettingRecords_select" id="lotno">
				      ...
				      <option value="J00001" <s:if test='request.getParameter("lotno")=="J00001"'>selected="selected"</s:if>>竞彩足球胜平负</option>
				      <option value="J00002" <s:if test='request.getParameter("lotno")=="J00002"'>selected="selected"</s:if>>竞彩足球比分</option>
				      <option value="J00003" <s:if test='request.getParameter("lotno")=="J00003"'>selected="selected"</s:if>>竞彩足球总进球</option>
				      <option value="J00004" <s:if test='request.getParameter("lotno")=="J00004"'>selected="selected"</s:if>>竞彩足球半场胜平负</option>
				      <option value="J00005" <s:if test='request.getParameter("lotno")=="J00005"'>selected="selected"</s:if>>竞彩篮球胜负</option>
				      <option value="J00008" <s:if test='request.getParameter("lotno")=="J00008"'>selected="selected"</s:if>>竞彩篮球大小分</option>
				      <option value="J00007" <s:if test='request.getParameter("lotno")=="J00007"'>selected="selected"</s:if>>竞彩篮球胜分差</option>
				      <option value="J00006" <s:if test='request.getParameter("lotno")=="J00006"'>selected="selected"</s:if>>竞彩篮球让分胜负</option>
		...
		<td>起始日期：</td>
		<td><input class="account_time" type="text" id="beginTime" size="10" name="beginTime" onfocus="WdatePicker({alwaysUseStartDate:true})" value='<s:property value="#session.beginTime"/>'/></td>
		<td>终止日期：</td>
		<td><input id="endTime" size="10" name="endTime" onfocus="WdatePicker({alwaysUseStartDate:true})" value='<s:property value="#session.endTime"/>' type="text" class="account_time" /></td>
...
<table class="account_day"  cellspacing="0" cellpadding="0">
	...
	<s:else><tr class="accountDay_greyBg"></s:else>
	    ...
	    <s:elseif test='lotno=="J00001"'>竞彩足球胜平负</s:elseif>
	    <s:elseif test='lotno=="J00002"'>竞彩足球比分</s:elseif>
	    <s:elseif test='lotno=="J00003"'>竞彩足球总进球</s:elseif>
	    <s:elseif test='lotno=="J00004"'>竞彩足球半场胜平负</s:elseif>
	    <s:elseif test='lotno=="J00005"'>竞彩篮球胜负</s:elseif>
	    <s:elseif test='lotno=="J00008"'>竞彩篮球大小分</s:elseif>
	    <s:elseif test='lotno=="J00007"'>竞彩篮球胜分差</s:elseif>
	    <s:elseif test='lotno=="J00006"'>竞彩篮球让分胜负</s:elseif>
		...
		<%/* 期号  */ %>
		<td><s:if test='lotno=="J00001"||lotno=="J00002"||lotno=="J00003"||lotno=="J00004"||lotno=="J00005"||lotno=="J00006"||lotno=="J00007"||lotno=="J00008"||batchcode.equals("null")'>-</s:if><s:else><s:property value="batchcode"/></s:else></td>
		<%/* 发起人   */ %>
		...
		<td>
		<s:if test='bettype==3 && tlotcaseid != null && !tlotcaseid.equals("null")'>
		<a href='/chlw/function/selectAll!getCaseDetail?caselotId=<s:property value="tlotcaseid"/>'  target="_blank">查看</a>
		</s:if>
		<s:else>
		<a href='/rules/oneBetInfo.html?flowno=<s:property value="id"/>&play_name=<s:property value="lotno"/>&batchcode=<s:property value="batchcode"/>'  target="_blank">查看</a>
		</s:else>
		</td>
	</tr>
	</s:iterator>
...
<p class="account_page"> 	
	...
	<div class="space10">&#160;</div>
	 ...
	 <% /* <s:if test='#request.inDealMoney==null'>
    	 <div class="deal">支出交易笔数：0　　　　　支出金额合计：
<font class="red1">0</font>元<br />收入交易笔数：0　　　　　收入金额合计：<font class="red1">0</font>元
</div>
     </s:if>
     <s:else>
      <div class="deal">支出交易笔数：<s:property value="#request.inDealMoney.outnum"/>　　　　　支出金额合计：
<font class="red1"><s:property value="#request.inDealMoney.outamt/100"/></font>元<br />收入交易笔数：<s:property value="#request.inDealMoney.innum"/>　　　　　收入金额合计：
<font class="red1"><s:property value="#request.inDealMoney.inamt/100"/></font>元</div>
</s:else>
     */ %>
