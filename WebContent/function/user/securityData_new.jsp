...
<%if (tuser.getNICKNAME()==null || tuser.getNICKNAME().trim()=="" || tuser.getNAME()==null || tuser.getNAME().trim()=="" || tuser.getCERTID()==null || tuser.getCERTID().trim()=="" || tuser.getCERTID()=="111111111111111111" || tuser.getMOBILEID()==null || tuser.getMOBILEID().trim()=="" || tuser.getEMAIL()==null || tuser.getEMAIL().trim()=="" || tuser.getEMAIL().indexOf("@")==-1){ %>
	<div class="safetyInfor_title">为了您的账户安全，请如实填写以下资料</div>
...
<%-- 			<%if (tuser.getMOBILEID()!=null && !"".equals(tuser.getMOBILEID().trim())) {%> --%>
<%-- 			<td class="safetyInfor_link"><input name="" type="hidde" class="disabled_mailAddress" id="mobile" disabled="disabled" value='<%=tuser.getMOBILEID().substring(0,7)+"****" %>'/></td><td class="safetyInfor_link" width="300"><a href="javascript:phoneUnBundling();" title="解绑">解绑</a></td> --%>
<%-- 			<% }else{ %> --%>
<%-- 			<td class="safetyInfor_words" colspan="2"><input name="" type="text" class="mailAddress" id="mobile" onblur="checkMobile()"/><span id="mobileImage"></span><span class="account_modifyPassword" id="mobileTip">中奖后会通过手机短信免费通知您</span></td> --%>
<%-- 			<%} %> --%>
<!-- 		</tr> -->
		<tr>
			<th width="118">电子邮箱</th>
			<%if (tuser.getEMAIL()!=null && !"".equals(tuser.getEMAIL().trim()) && tuser.getEMAIL().indexOf("@")!=-1 ) {%>
			<td class=safetyInfor_link><input name="" type="text" class="disabled_mailAddress" disabled="disabled" id="email" value='<%= tuser.getEMAIL().trim().split("@")[0].substring(0,2)+"***@"+tuser.getEMAIL().trim().split("@")[1] %>'/></td><td class="safetyInfor_link" width="300"><a href="/rules/user.html?key=38" title="修改">修改</a></td>
			<% }else{ %>
