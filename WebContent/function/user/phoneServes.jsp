...
<%	
	...
	if(tuserinfo != null && tuserinfo.getJSONObject("value") != null){
	 tuser = Tuserinfo.setJson(tuserinfo.getJSONObject("value"));
	}
	String mobile = tuser.getMOBILEID();
%>
<h2>手机服务</h2>
<%if (mobile==null || mobile.length()<5){ %>
	<div class="accountService_prompt">温馨提示：您尚未绑定手机！现在就绑定手机，之后即可<b>免费定制</b>以下短信服务，绑定手机无需任何费用。<a href="/rules/user.html?key=50" title="立即绑定">立即绑定>></a></div>
...
<%}else{ %>
	...
	<%if (mobile==tuser.getUSERNAME()){ %><%= mobile.substring(0,7)+"****" %><%}else{ %><%= mobile.substring(0,7)+"****" %><a href="javascript:phoneUnBundling();" title="我要解绑"><img src="../images/Solution.gif" /></a><%} %></div>
	<div class="accountService_binding">
		<div class="accountService_checkbox">
			<font><img src="../images/myAccount_checkBox.gif" width="13" height="13" /></font>
		...
		<dl class="CheckBox" value="4" >
			...
			<dd class="accountService_bindingText">当你定制的追号（期数≥10）还剩三期时，短信提醒您。</dd>
			<dd class="accountService_bindingImg"></dd>
			<dd class="huizi" ><span  id="info_qxzh">已定制</span>   <span> 此服务完全<b>免费</b></span></dd>
		</dl>
	</div>
<%} %>
